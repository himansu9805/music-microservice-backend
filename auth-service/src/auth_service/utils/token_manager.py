...
def decode_token(token: str, refresh: bool = False):
    ...
    Args:
        ...
        refresh (bool, optional): Flag to indicate if the refresh token secret key should be used. Defaults to False.

    Returns:
        dict or None: The decoded JWT payload if valid and not expired, otherwise None.
    ...
    try:
        ...
        return decoded_jwt if decoded_jwt["exp"] >= datetime.now().timestamp() else None
